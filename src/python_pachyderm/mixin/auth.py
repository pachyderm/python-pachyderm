...
class AuthMixin:
    def activate_auth(self, subject=None, github_token=None):
        ...
        * An optional string. This is the token returned by GitHub and used to
        ...
        """
        token = self._req(Service.AUTH, "Activate", subject=subject, github_token=github_token).pach_token
    ...
    def authenticate_github(self, github_token):
        ...
        * `github_token`: A string. This is the token returned by GitHub and
        ...
        retrieve the corresponding GitHub username.)
        """
        token = self._req(Service.AUTH, "Authenticate", github_token=github_token).pach_token
        self.metadata = [(k, v) for (k, v) in self.metadata if k != "authn-token"]
        self.metadata.append(("authn-token", token))
        return token
    ...
    def authenticate_one_time_password(self, one_time_password):
        ...
        * `one_time_password`: A string. This is a short-lived, one-time-use
        ...
        """
        token = self._req(Service.AUTH, "Authenticate", one_time_password=one_time_password).pach_token
        self.metadata = [(k, v) for (k, v) in self.metadata if k != "authn-token"]
        self.metadata.append(("authn-token", token))
        return token
